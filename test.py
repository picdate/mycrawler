...
def getRegResualt(str,expression):
    pattern=re.compile(expression)
    res=pattern.search(str)
    if res==None :return res
    else :return res.group()


def xxxswitch_crawler():

    print("-----输入爬取范围-----  (?,?)")
    range_str=input()
    if(getRegResualt(range_str,"^\([\d]+,[\d]+\)$")==None):
        print("范围格式错误") 
        return None
    else:
       left=""
       right=""
       temp=""
       for i in range(0,range_str.__len__()):
            if(range_str[i]=="("):
                continue
            if(range_str[i]==","):
                left=int(temp)
                temp=""
                continue
            if(ord(range_str[i])<=57 and ord(range_str[i])>=48):
                temp=temp+range_str[i].__str__()
       right=int(temp)
